...
sealed class ScanningError(val unlocalizedMessage: String) {
    data class UnsupportedContentType(val contentType: String) :
        ScanningError(
            "Unsupported content type: $contentType"
        )
    data class InvalidScanHandle(val handle: ScannerHandle) :
        ScanningError(
            "Invalid scan handle: $handle"
        )
    data class JobCreationFailed(val creationError: String) :
        ScanningError(
            "Job creation failed: $creationError"
        )

    data class JobCompletedInFailedState(val jobStatus: String) :
        ScanningError(
            "Job completed but failed. Job status: $jobStatus"
        )

    data class CannotGetScannerCaps(val error: String) :
        ScanningError(
    ...
    data class NextPageRetrievalError(val error: String, val jobStatus: String) :
        ScanningError(
