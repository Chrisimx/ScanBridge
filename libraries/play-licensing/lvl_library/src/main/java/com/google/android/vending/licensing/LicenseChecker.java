...
public class LicenseChecker implements ServiceConnection {
    ...
    public void checkAccess(LicenseCheckerCallback callback) {
        checkAccess(generateNonce(), callback);
    }
     ...
     * @param callback
     */
    public synchronized void checkAccess(int nonce, LicenseCheckerCallback callback) {
        // If we have a valid recent LICENSED response, we can skip asking
        // Market.
        if (mPolicy.allowAccess()) {
            Log.i(TAG, "Using cached license response");
            callback.allow(LicenseValidationResultCode.LICENSED);
        } else {
            LicenseValidator validator = new LicenseValidator(mPolicy, new NullDeviceLimiter(),
                    callback, nonce, mPackageName, mVersionCode);
            ...
            if (mService == null) {
                ...
                try {
                    ...
                    callback.applicationError(LicenseValidationResultCode.ERROR_MISSING_PERMISSION);
                } catch (Base64DecoderException e) {
                    e.printStackTrace();
                }
            } else {
    ...
    private synchronized void handleServiceConnectionError(LicenseValidator validator) {
        mPolicy.processServerResponse(Policy.RETRY,
                null, null,
                LicenseValidationResultCode.GOOGLE_PLAY_SERVICE_CONNECTION_FAILED
        );

        if (mPolicy.allowAccess()) {
            validator.getCallback().allow(LicenseValidationResultCode.GOOGLE_PLAY_SERVICE_CONNECTION_FAILED);
        } else {
            validator.getCallback().dontAllow(LicenseValidationResultCode.GOOGLE_PLAY_SERVICE_CONNECTION_FAILED);
        }
    }
